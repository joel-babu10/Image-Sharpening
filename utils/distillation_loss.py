...
class KnowledgeDistillationLoss(nn.Module):
    def __init__(self, alpha=0.6, beta=0.2, gamma=0.1, delta=0.1):
        ...
        self.reconstruction = ReconstructionLoss('l1')
        self.perceptual = PerceptualLoss(layer=8)
        self.feature_distill = FeatureDistillationLoss()
        self.edge = EdgeLoss()

    def forward(self, student_out, target_hr, teacher_out,
                student_feats=None, teacher_feats=None):

        loss = 0.0
        loss += self.alpha * self.reconstruction(student_out, target_hr)
        loss += self.beta * self.perceptual(student_out, teacher_out)
        loss += self.delta * self.edge(student_out, target_hr)

        if student_feats is not None and teacher_feats is not None:
            loss += self.gamma * self.feature_distill(student_feats, teacher_feats)
